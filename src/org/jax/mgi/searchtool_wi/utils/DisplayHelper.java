...
public class DisplayHelper
{
	...
	*/
	...
	public DisplayHelper(Configuration c , GenomeFeatureDisplayCache mdc, VocabDisplayCache vdc) {
		stConfig = c;
...
/*		String warningIcon = "<img src=" + stConfig.get("QUICKSEARCH_URL")
	...
	public String getStemmedSearchTermDisplay(SearchInput si) throws IOException {
		...
		if (stemmedTokenizedInputString.size() > 0 && !si.hasPrefix()) {
			...
			for (Iterator<String> iter = stemmedTokenizedInputString.iterator(); iter.hasNext();) {
		String token = (String) iter.next();
		if (token.contains("\"")) {
	...
	public String getSearchDetailOnClick(SearchInput si) throws IOException {
		...
		if (tokenizedInputString.size() > 0) {
			...
			if (zeroHitTokens.size() > 0) {
					...
					int first = 0;
				for (Iterator<String> iter = zeroHitTokens.iterator(); iter.hasNext();) {
					String token = (String) iter.next();
	...
	*/
	public static String getMarkerScoreMouseOver (GenomeFeatureResult genomeFeatureResult) {
		...
		String caption = "see matches for " + DisplayHelper.superscript(markerDisplay.getSymbol());
		String contents = "";
...
//			url ="<a href = '" + stConfig.get("FEWI_URL") + "gxd/summary?structureKey=" + vocabDisplay.getDbKey() + "'>" + vocabDisplay.getAnnotDisplay()+ "</a>";
		if (vocab.equals(IndexConstants.MP_TYPE_NAME)) {
			...
			url = "<a href='"+stConfig.get("FEWI_URL")+"vocab/gxd/anatomy/" + vocabDisplay.getAcc_id() + "'> "+ vocabDisplay.getAnnotDisplay()+"</a>";
